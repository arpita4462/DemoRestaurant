...
import com.gmediasolutions.demorestaurant.alert.CheckPermission
...
/*  private fun attemptIntentLogin(userslogin: LoginRequest) {
      ...
      postUser.queue(object : Callback<ApiReturn> {
...
*/
  ...
  private fun validatePassword(pass: String): Boolean {
      ...
      if (pass.length < 4 || pass.length > 20) {
          input_layout_pass.setError("Password Must consist of 4 to 20 characters")
          input_layout_email.setErrorEnabled(false)
          input_layout_pass.requestFocus()
          return false
      }
      return true
  }
  ...
  private fun validateUsername(email: String): Boolean {
      if (email.length < 4 || email.length > 30) {
          input_layout_email.setError("Email Must consist of 4 to 30 characters")
          input_layout_email.requestFocus()
          return false
      } else if (!email.matches("^[A-za-z0-9.@]+".toRegex())) {
          input_layout_email.setError("Only . and @ characters allowed")
          input_layout_email.requestFocus()
          return false
      } else if (!email.contains("@") || !email.contains(".")) {
          input_layout_email.setError("Email must contain @ and .")
          input_layout_email.requestFocus()
          return false
      }
      return true
